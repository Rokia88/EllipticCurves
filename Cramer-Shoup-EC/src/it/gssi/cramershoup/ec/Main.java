...
public class Main {
	...
	public static void main(String[] args) throws NoCommonMotherException, NotOnMotherException, FileNotFoundException, UnsupportedEncodingException {
		try {
			...
			double durationD;
			long start;
			PrintWriter writer = new PrintWriter("ExecutionTimeCS-EC.txt", "UTF-8");
			...
			for(int i = 0; i < keyL.length; i++)
			{
					sec = createEC(keyL[i]);
				 	...
				 	CramerSCryptoSystem cs = new CramerSCryptoSystem(ec);

				    CramerSKey sk = (CramerSKey)cs.generateKey();
				    CramerSKey pk = sk.getPublic();
				    ...
				    for(int k = 0; k < NbrTrials; k++)
				    {
					    ...
					    if(x1 != null &&  x.getx().compareTo(x1.getx()) == 0 && x.gety().compareTo(x1.gety()) == 0)
					    {
